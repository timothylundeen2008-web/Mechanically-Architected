...
def empty_snapshot_message(error: str) -> None:
    st.error(f"Live data is temporarily unavailable: {error}")
    st.info(
        "Try Recalculate again in a minute. If FRED is having a temporary server issue, "
        "the app should recover automatically on the next refresh."
    )
...
with left:
    ...
    st.markdown(
        f'<div class="app-updated">Live data refreshes automatically every day; last successful fetch shown below.</div>',
        unsafe_allow_html=True,
    )
with right:
...
if error and snapshot is None:
    empty_snapshot_message(error)
    st.stop()
elif error and snapshot is not None:
    st.warning(f"Live update failed, showing last good cached data instead: {error}")
