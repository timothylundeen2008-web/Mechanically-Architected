...
from storage_backend import read_json, write_json, _bridge_repo
...
def publish(regime: dict, repression: Optional[dict] = None,
    ...
    # BRIDGE_REPO override, not this app's own GITHUB_REPO. Markets'
    # GITHUB_REPO correctly points at Mechanically-Architected (for THIS
    # app's own logs); if markets_summary.json also wrote there, All-Weather
    # reading with ITS OWN GITHUB_REPO (Portfolio-Tracker) would never find
    # it — one secret cannot mean two different repos at once. See
    # storage_backend._bridge_repo() for the full explanation.
    res = write_json(MARKETS_FILE, payload,
                     message=f"markets: publish summary "
                             f"{payload['published_at']}",
                     repo_override=_bridge_repo())
...
def read() -> dict:
    ...
    """
    data = read_json(MARKETS_FILE, repo_override=_bridge_repo())
    if not data:
