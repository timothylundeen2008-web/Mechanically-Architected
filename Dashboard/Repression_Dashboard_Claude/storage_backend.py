...
def _bridge_repo() -> str:
    """
    The repo a CROSS-DASHBOARD bridge file lives in.

    One GITHUB_REPO secret cannot mean two things at once: "where this app's
    own data lives" (needed for log_viewer to find ITS OWN logs) and "where
    the shared bridge file lives" (needed for markets_bridge/rotation_bridge
    to agree across three separately-deployed apps with three separate
    secrets boxes). Markets' GITHUB_REPO correctly points at
    Mechanically-Architected for its own logs; if markets_summary.json also
    wrote there, All-Weather's markets_bridge.read() -- using ITS OWN
    GITHUB_REPO, pointing at Portfolio-Tracker -- would never find it.

    BRIDGE_REPO is an explicit, OPTIONAL override read by the bridge
    functions specifically. Set it to the SAME value in all three apps'
    secrets (whichever repo you designate as the shared bridge location --
    Portfolio-Tracker is the natural choice, since rotation_bridge already
    writes there). Falls back to GITHUB_REPO when unset, so anything that
    doesn't care about this distinction is unaffected.
    """
    return _cfg("BRIDGE_REPO") or _cfg("GITHUB_REPO")


def _gh_get(path: str, repo: str | None = None) -> tuple[str | None, str | None]:
    """Returns (content, sha). sha is required to update an existing file."""
    token = _cfg("GITHUB_TOKEN")
    repo = repo or _cfg("GITHUB_REPO")
...
def _gh_put(path: str, content: str, message: str,
            repo: str | None = None) -> bool:
    token = _cfg("GITHUB_TOKEN")
    repo = repo or _cfg("GITHUB_REPO")
    branch = _cfg("GITHUB_BRANCH", "main")
    _, sha = _gh_get(path, repo=repo)                 # need current sha to update
    url = f"{GITHUB_API}/repos/{repo}/contents/{path}"
...
def read_text(name: str, repo_override: str | None = None) -> str | None:
    ...
    if status["backend"] == "github":
        content, _ = _gh_get(path, repo=repo_override)
...
def write_text(name: str, content: str, message: str | None = None,
              repo_override: str | None = None) -> dict:
    """
    ...
    if backend_status()["backend"] == "github":
        res["github"] = _gh_put(path, content, msg, repo=repo_override)
...
def read_json(name: str, repo_override: str | None = None) -> dict | None:
    txt = read_text(name, repo_override=repo_override)
    if not txt:
        return None
    try:
...
def write_json(name: str, obj: dict, message: str | None = None,
              repo_override: str | None = None) -> dict:
