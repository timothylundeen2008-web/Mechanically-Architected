...
# ─── Yield Curve & Credit Conditions section ──────────────────────────────────

def _fetch_fred_inline(series_id: str, start_date: str = "2000-01-01") -> pd.Series:
    """
    Inline FRED fetch with session-state caching, mirroring the pattern already
    used for H.4.1 balance sheet series. Keeps a dedicated cache key per series
    so this section doesn't collide with the main indicator_data cache.
    """
    cache_key = f"fred_extra_{series_id}"
...
def yield_curve_credit_section(raw: dict):
    """
    2s10s spread history (with NBER recession shading) + investment-grade vs.
    high-yield credit spread comparison. Fetched inline via FRED so this section
    has no dependency on data_fetcher.py / indicators.py being modified.
    """
    st.markdown("**Yield curve & credit conditions**")
    st.markdown(
        "<span style='color:#9aa3b2;font-size:.82rem;'>"
        "The 2s10s spread is the most-watched recession signal in macro — every U.S. recession "
        "since the 1970s was preceded by an inversion (2-yr yield &gt; 10-yr yield). "
        "Credit spreads show whether bond investors are demanding more compensation for risk."
        "</span>",
        unsafe_allow_html=True
    )
    ...
    # ── 2s10s spread with recession shading ────────────────────────────────────
    with cc1:
        st.markdown("**2s10s Spread (10-yr minus 2-yr)**")

        t2_series  = raw.get("treasury_2y_series",  pd.Series(dtype=float))
        t10_series = raw.get("treasury_10y_series", pd.Series(dtype=float))

        if t2_series is not None and t10_series is not None and \
           len(t2_series) > 0 and len(t10_series) > 0:

            spread = (t10_series - t2_series).dropna()
            spread = spread.tail(1260)  # ~5y of trading days

            recession = _fetch_fred_inline("USREC", "2019-01-01")

            fig = go.Figure()

            # Recession shading (USREC is 0/1; shade where value == 1)
            if recession is not None and len(recession) > 0:
                rec = recession.dropna()
                rec = rec[rec.index >= spread.index.min()] if len(spread) > 0 else rec
                in_rec = False
                start = None
                for dt_, val in rec.items():
                    if val >= 0.5 and not in_rec:
                        in_rec = True
                        start = dt_
                    elif val < 0.5 and in_rec:
                        in_rec = False
                        fig.add_vrect(x0=start, x1=dt_,
                                      fillcolor="rgba(224,82,82,0.10)",
                                      line_width=0, layer="below")
                if in_rec and start is not None:
                    fig.add_vrect(x0=start, x1=rec.index.max(),
                                  fillcolor="rgba(224,82,82,0.10)",
                                  line_width=0, layer="below")

            spread_color = "#5a9e47" if spread.iloc[-1] >= 0 else "#e05252"
            fig.add_trace(go.Scatter(
                x=spread.index, y=spread.values,
                line=dict(color=spread_color, width=1.8),
                fill="tozeroy",
                fillcolor=f"rgba({_hex_to_rgb(spread_color)},0.12)",
                hovertemplate="2s10s: %{y:.2f}%<extra></extra>",
                showlegend=False,
            ))
            fig.add_hline(y=0, line_dash="dot", line_color="#5c6475", line_width=1)

            fig.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(t=10, b=30, l=40, r=20), height=240,
                showlegend=False,
                xaxis=dict(showgrid=False, tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830"),
                yaxis=dict(gridcolor="#1a1e25", tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830", ticksuffix="%"),
                hovermode="x unified",
            )
            st.plotly_chart(fig, use_container_width=True,
                            config={"displayModeBar": False})
            st.markdown(
                "<span style='font-size:.72rem;color:#5c6475;'>"
                "Shaded bands = NBER recession periods (FRED: USREC). "
                "Below the dotted zero line = inverted curve."
            ...
            st.markdown(
                ...
                'Treasury yield series unavailable — check FRED connection</div>',
                unsafe_allow_html=True
            )

    # ── IG vs HY credit spreads ──────────────────────────────────────────────────
    with cc2:
        st.markdown("**Credit Spreads — Investment Grade vs. High Yield**")

        ig_series = _fetch_fred_inline("BAMLC0A0CM", "2019-01-01")    # IG OAS
        hy_series = raw.get("hy_spread_series", pd.Series(dtype=float))
        if hy_series is None or len(hy_series) == 0:
            hy_series = _fetch_fred_inline("BAMLH0A0HYM2", "2019-01-01")

        if (ig_series is not None and len(ig_series) > 0) or \
           (hy_series is not None and len(hy_series) > 0):

            fig2 = go.Figure()
            if hy_series is not None and len(hy_series) > 0:
                hy_trim = hy_series.dropna().tail(1260)
                fig2.add_trace(go.Scatter(
                    x=hy_trim.index, y=hy_trim.values,
                    name="High Yield OAS",
                    line=dict(color="#e05252", width=1.8),
                    hovertemplate="HY OAS: %{y:.2f}%<extra></extra>",
                ))
            if ig_series is not None and len(ig_series) > 0:
                ig_trim = ig_series.dropna().tail(1260)
                fig2.add_trace(go.Scatter(
                    x=ig_trim.index, y=ig_trim.values,
                    name="Investment Grade OAS",
                    line=dict(color="#4a8fd4", width=1.8),
                    yaxis="y2",
                    hovertemplate="IG OAS: %{y:.2f}%<extra></extra>",
                ))

            hy_vals = hy_series.dropna().tail(1260).values if hy_series is not None and len(hy_series) > 0 else []
            ig_vals = ig_series.dropna().tail(1260).values if ig_series is not None and len(ig_series) > 0 else []

            fig2.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(t=10, b=30, l=40, r=45), height=240,
                showlegend=True,
                legend=dict(orientation="h", yanchor="bottom", y=1.01,
                           xanchor="left", x=0, font=dict(color="#9aa3b2", size=10),
                           bgcolor="rgba(0,0,0,0)"),
                xaxis=dict(showgrid=False, tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830"),
                yaxis=dict(title="HY OAS (%)", title_font=dict(color="#e05252", size=10),
                           gridcolor="#1a1e25", tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830", ticksuffix="%",
                           range=[0, max(hy_vals.max() * 1.15, 6)] if len(hy_vals) else None),
                yaxis2=dict(title="IG OAS (%)", title_font=dict(color="#4a8fd4", size=10),
                           overlaying="y", side="right", showgrid=False,
                           tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830", ticksuffix="%",
                           range=[0, max(ig_vals.max() * 1.4, 2)] if len(ig_vals) else None),
                hovermode="x unified",
            )
            ...
            st.markdown(
                "<span style='font-size:.72rem;color:#5c6475;'>"
                "Widening spreads = credit stress / risk-off. HY widens faster and harder "
                "than IG in a downturn — watch for divergence between the two."
                "</span>", unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<div style="background:#13161b;border:1px solid #242830;border-radius:10px;'
                'padding:2rem;text-align:center;color:#5c6475;font-size:.8rem;">'
                'Credit spread data unavailable — check FRED connection</div>',
                unsafe_allow_html=True
            )


# ─── Dollar, Debt & Gold Confirmation section ─────────────────────────────────

def dollar_debt_gold_section(raw: dict):
    """
    Three live panels added for the DXY/real-yield/gold-gate/auction-demand
    signal set:
      1) DXY vs. 10-yr real yield (DFII10) overlay — watch for the two lines
    ...
    """
    st.markdown("**Dollar, debt & gold confirmation**")
    st.markdown(
        "<span style='color:#9aa3b2;font-size:.82rem;'>"
    ...
    # ── DXY vs. 10-yr real yield overlay ───────────────────────────────────────
    with dc1:
        st.markdown("**DXY vs. 10-yr Real Yield (DFII10)**")

        dxy_series  = raw.get("dxy_series",          pd.Series(dtype=float))
        real_series = raw.get("tips_real_yield_series", pd.Series(dtype=float))

        if len(dxy_series) > 0 and len(real_series) > 0:
            dxy_trim  = dxy_series.dropna().tail(1260)
            real_trim = real_series.dropna().tail(1260)

            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=dxy_trim.index, y=dxy_trim.values,
                name="DXY", line=dict(color="#9aa3b2", width=1.8),
                hovertemplate="DXY: %{y:.2f}<extra></extra>",
            ))
            fig.add_trace(go.Scatter(
                x=real_trim.index, y=real_trim.values,
                name="10-yr Real Yield (DFII10)", yaxis="y2",
                line=dict(color="#4a8fd4", width=1.8),
                hovertemplate="Real yield: %{y:.2f}%<extra></extra>",
            ))
            fig.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(t=10, b=30, l=45, r=45), height=240,
                legend=dict(orientation="h", yanchor="bottom", y=1.01,
                           xanchor="left", x=0, font=dict(color="#9aa3b2", size=10),
                           bgcolor="rgba(0,0,0,0)"),
                xaxis=dict(showgrid=False, tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830"),
                yaxis=dict(title="DXY", title_font=dict(color="#9aa3b2", size=10),
                           gridcolor="#1a1e25", tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830"),
                yaxis2=dict(title="Real yield (%)", title_font=dict(color="#4a8fd4", size=10),
                           overlaying="y", side="right", showgrid=False,
                           tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830", ticksuffix="%"),
                hovermode="x unified",
            ...
            if dxy_chg is not None:
                clr = "#e05252" if dxy_chg < -1.5 else "#9aa3b2"
                st.markdown(
                    f"<span style='font-size:.72rem;color:#5c6475;'>"
                    f"DXY 20-trading-day change: <b style='color:{clr};'>{dxy_chg:+.1f}%</b>. "
                    "Lines decoupling (real yield rising, DXY falling) = debt-confidence signal, "
                    "not a normal growth/policy move."
                    "</span>", unsafe_allow_html=True
                )
        else:
            st.markdown(
                '<div style="background:#13161b;border:1px solid #242830;border-radius:10px;'
                'padding:2rem;text-align:center;color:#5c6475;font-size:.8rem;">'
                'DXY or real yield series unavailable — check network/FRED connection</div>',
                unsafe_allow_html=True
            )

    # ── Treasury 10-yr auction bid-to-cover trend ──────────────────────────────
    with dc2:
        st.markdown("**10-yr Note Auction Bid-to-Cover**")

        btc_series = raw.get("auction_10y_btc_series", pd.Series(dtype=float))

        if len(btc_series) > 0:
            colors = ["#e05252" if v < 2.0 else "#d4913a" if v < 2.3 else "#5a9e47"
                     for v in btc_series.values]
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(
                x=[d.strftime("%b %y") for d in btc_series.index],
                y=btc_series.values,
                marker_color=colors,
                hovertemplate="B/C: %{y:.2f}<extra></extra>",
            ))
            fig2.add_hline(y=2.3, line_dash="dot", line_color="#5c6475", line_width=1,
                           annotation_text="  healthy: 2.3+",
                           annotation_font_color="#5c6475", annotation_font_size=10)
            fig2.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(t=10, b=30, l=40, r=20), height=240,
                showlegend=False,
                xaxis=dict(showgrid=False, tickfont=dict(color="#5c6475", size=9),
                           linecolor="#242830"),
                yaxis=dict(title="Bid-to-cover", title_font=dict(color="#9aa3b2", size=10),
                           gridcolor="#1a1e25", tickfont=dict(color="#5c6475", size=10),
                           linecolor="#242830"),
            )
            st.plotly_chart(fig2, use_container_width=True,
            ...
            st.markdown(
                "<span style='font-size:.72rem;color:#5c6475;'>"
                "Source: U.S. Treasury Fiscal Data API. Below 2.3 = softening demand, "
                "below 2.0 = genuinely weak auction — a leading debt-sustainability signal."
                "</span>", unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<div style="background:#13161b;border:1px solid #242830;border-radius:10px;'
                'padding:2rem;text-align:center;color:#5c6475;font-size:.8rem;">'
                'Auction data unavailable — Treasury Fiscal Data API may be unreachable</div>',
                unsafe_allow_html=True
            )
    ...
    tips = raw.get("tips_real_yield")
    be_series = raw.get("breakeven_series", pd.Series(dtype=float))

    if len(gld.dropna()) >= 200 and tips is not None:
        g = gld.dropna()
        ma50, ma200 = float(g.tail(50).mean()), float(g.tail(200).mean())
        price_gate = ma50 > ma200
        be_rising = None
        if len(be_series.dropna()) >= 11:
            be = be_series.dropna()
            be_rising = be.iloc[-1] > be.iloc[-11]
        fundamental_gate = (tips < 1.5) and bool(be_rising)
        both = fundamental_gate and price_gate

        g1, g2, g3 = st.columns(3)
        for col, label, is_open, detail in [
            (g1, "Fundamental gate", fundamental_gate,
             f"Real yield {tips:.2f}% (<1.5 needed), breakevens {'rising' if be_rising else 'flat/falling'}"),
            (g2, "Price gate", price_gate,
             f"GLD 50d ${ma50:.2f} {'>' if price_gate else '<'} 200d ${ma200:.2f}"),
            (g3, "Combined", both,
             "Both open — confirmed signal" if both else "Not fully confirmed"),
        ]:
            clr = "#e05252" if is_open else "#5a9e47"
            state = "OPEN" if is_open else "closed"
            col.markdown(
                f'<div style="background:#13161b;border:1px solid #242830;'
                f'border-radius:8px;padding:.65rem .85rem;text-align:center;">'
                f'<div style="font-size:1rem;font-weight:700;color:{clr};">{state}</div>'
                f'<div style="font-size:.7rem;color:#9aa3b2;margin-top:2px;">{label}</div>'
                f'<div style="font-size:.65rem;color:#5c6475;margin-top:4px;">{detail}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
    else:
        st.markdown(
            '<div style="background:#13161b;border:1px solid #242830;border-radius:10px;'
            'padding:1.2rem;text-align:center;color:#5c6475;font-size:.8rem;">'
            'Gold gate unavailable — need 200+ days of GLD price history and a real yield reading</div>',
            unsafe_allow_html=True
        )


# ─── ACM Term Premium section (manual CSV upload) ─────────────────────────────

def _parse_acm_term_premium_csv(file) -> pd.DataFrame | None:
    """
    Parses the NY Fed's ACM Term Premium CSV (downloaded manually from
    newyorkfed.org/research/data_indicators/term-premia-tabs).

    Expected format: a DATE column plus ACMY01..ACMY10 (fitted yields),
    ACMTP01..ACMTP10 (term premia), ACMRNY01..ACMRNY10 (risk-neutral yields).
    Falls back to a looser column match if the NY Fed changes header naming.
    """
    try:
        df = pd.read_csv(file)
    except Exception:
    ...
    date_col = None
    for c in df.columns:
        if str(c).strip().upper() in ("DATE", "DATES"):
            date_col = c
            break
    if date_col is None:
    ...
    # Find the 10-yr term premium column (standard name: ACMTP10)
    tp10_col = None
    for c in df.columns:
        ...
        if c_clean == "ACMTP10":
            ...
            break
    if tp10_col is None:
        # Loose fallback — any column containing both "TP" and "10"
        candidates = [c for c in df.columns if "TP" in str(c).upper() and "10" in str(c)]
        tp10_col = candidates[0] if candidates else None

    if tp10_col is None:
        return None

    out = pd.DataFrame({"term_premium_10y": pd.to_numeric(df[tp10_col], errors="coerce")})
    out = out.dropna()
    return out if len(out) > 0 else None


def term_premium_section():
    """
    ACM 10-yr Term Premium — manual CSV upload, since the NY Fed does not
    expose a clean public API for this series (CSV-only, updated weekly).
    Decomposes the 10-yr yield into Fed-expectation vs. term-premium components.
    """
    st.markdown("**10-yr Term Premium (NY Fed ACM Model)**")
    st.markdown(
        "<span style='color:#9aa3b2;font-size:.82rem;'>"
        "Isolates how much of the 10-yr yield is compensation for duration risk, versus "
        "the market's expected path of short-term rates. Rising term premium with a stable "
        "Fed outlook signals the market wants more compensation for buying long bonds — often "
        "tied to fiscal/issuance concerns rather than Fed policy."
        "</span>",
        unsafe_allow_html=True
    )

    with st.expander("📤 Upload ACM Term Premium CSV", expanded=("term_premium_df" not in st.session_state)):
        st.markdown(
            "<span style='font-size:.78rem;color:#9aa3b2;'>"
            "No public API exists for this series — download the CSV directly from the NY Fed, then upload it here. "
            "It updates roughly weekly.<br>"
            "<a href='https://www.newyorkfed.org/research/data_indicators/term-premia-tabs#/interactive' "
            "style='color:#4a8fd4;'>Get the CSV from the NY Fed →</a>"
            "</span>",
            unsafe_allow_html=True
        )
        uploaded = st.file_uploader(
            "ACM Term Premium CSV", type=["csv"], key="acm_csv_upload",
            label_visibility="collapsed",
        )
        if uploaded is not None:
            parsed = _parse_acm_term_premium_csv(uploaded)
            if parsed is not None:
                st.session_state["term_premium_df"] = parsed
                st.success(f"✅ Loaded {len(parsed)} rows — latest: "
                          f"{parsed.index[-1].date()} = {parsed['term_premium_10y'].iloc[-1]:+.2f}%")
            else:
                st.error(
                    "❌ Couldn't find a 10-yr term premium column (expected `ACMTP10`). "
                    "Check that this is the unmodified CSV from the NY Fed site."
                )

    tp_df = st.session_state.get("term_premium_df")
    if tp_df is not None and len(tp_df) > 0:
        tp_trim = tp_df["term_premium_10y"].tail(1260)
        latest_tp = tp_trim.iloc[-1]
        tp_color = "#e05252" if latest_tp > 1.0 else "#d4913a" if latest_tp > 0 else "#5a9e47"

        m1, m2 = st.columns([1, 3])
        with m1:
            st.markdown(
                f'<div class="metric-box">'
                f'<div class="metric-val" style="color:{tp_color};">{latest_tp:+.2f}%</div>'
                f'<div class="metric-label">10-yr Term Premium</div>'
                f'</div>', unsafe_allow_html=True
            )
        with m2:
            fig = make_history_chart(
                tp_trim, "10-yr Term Premium (ACM)", 0.0,
                "Zero line — negative = market subsidizing duration", "#c76bdb"
            )
            st.plotly_chart(fig, use_container_width=True,
                            config={"displayModeBar": False})
        st.markdown(
            f"<span style='font-size:.72rem;color:#5c6475;'>"
            f"Data as of {tp_trim.index[-1].date()} · Source: NY Fed ACM model (manual upload, not auto-refreshed)"
            f"</span>", unsafe_allow_html=True
        )
    else:
        st.markdown(
            '<div style="background:#13161b;border:1px solid #242830;border-radius:10px;'
            'padding:1.5rem;text-align:center;color:#5c6475;font-size:.8rem;">'
            'Upload the ACM CSV above to see the term premium chart</div>',
            unsafe_allow_html=True
        )
...
def main():
    ...
    with tab7:
        ...
        # question: "what macro environment is this?" -- the universal map
        # (growth x inflation) every allocator knows, and specifically the
        # intellectual foundation of the All-Weather portfolio itself.
        # The regime classifier then answers "what SPECIFICALLY is happening
        # inside that box right now." Both are needed; neither replaces the
        # other. Fetches NOTHING -- pure synthesis of the growth composite
        # and CPI that _regime_out already carries.
        try:
            import macro_quadrant as _mq
            _sig = (_regime_out or {}).get("signals")
            _q = _mq.classify(
            ...
            st.caption(f"⚠ Macro quadrant unavailable: {_e}")
    ...
    with tab2:
        ...
        yield_curve_credit_section(raw)

        st.markdown("---")

        # ── Dollar, Debt & Gold Confirmation ────────────────────────────────────
        dollar_debt_gold_section(raw)

        st.markdown("---")

        # ── ACM Term Premium (manual upload) ────────────────────────────────────
    ...
    st.markdown(
        ...
        "ICE BofA Bond Indices (via FRED) · NY Fed ACM Term Premium Model (manual upload).<br>"
