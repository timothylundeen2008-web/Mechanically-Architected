...
def render_progress(current_pct: float):
...
def render_streak_boxes(streak: int, target: int):
    boxes = []
    for idx in range(target):
        on = idx < streak
        content = "•" if not on else "✓"
        boxes.append(f'<span class="streak-box {"on" if on else ""}">{content}</span>')
    st.markdown(f'<div class="streak-row">{"".join(boxes)}</div>', unsafe_allow_html=True)


def render_signal_card_1(yield_value: float, streak: int, triggered: bool):
    st.markdown('<div class="signal-card">', unsafe_allow_html=True)
    st.markdown('<div class="signal-eyebrow">Signal 1 — Treasury</div>', unsafe_allow_html=True)
    st.markdown('<div class="signal-name">10-year yield</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="signal-value">{yield_value:.2f}%</div>', unsafe_allow_html=True)
    st.markdown('<div class="signal-sub">—</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="progress-label-row"><span>Alert threshold</span><span class="threshold-right">{TREASURY_THRESHOLD:.2f}%</span></div>',
        unsafe_allow_html=True,
    )
    render_progress(progress_pct(yield_value, TREASURY_THRESHOLD))
    st.markdown(
        f'<div class="progress-label-row"><span>Days above 5% (need {TREASURY_STREAK_TRIGGER} to trigger)</span></div>',
        unsafe_allow_html=True,
    )
    render_streak_boxes(streak, TREASURY_STREAK_TRIGGER)
    pill = "Treasury signal triggered" if triggered else "Below threshold"
    pill_cls = "pill pill-danger" if triggered else "pill"
    st.markdown(f'<span class="{pill_cls}">{pill}</span>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)


def render_signal_card_2(kre_value: float, baseline: float, triggered: bool):
    ...
    st.markdown('<div class="signal-card">', unsafe_allow_html=True)
    st.markdown('<div class="signal-eyebrow">Signal 2 — Regional Banks</div>', unsafe_allow_html=True)
    st.markdown('<div class="signal-name">KRE ETF</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="signal-value">${kre_value:,.2f}</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="signal-sub">{signed_pct_text(decline_pct)} from baseline (${baseline:,.2f})</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="progress-label-row"><span>Alert level (-30%)</span><span class="threshold-right">${alert_level_price:,.2f}</span></div>',
        unsafe_allow_html=True,
    )
    ...
    pill_cls = "pill pill-danger" if triggered else "pill"
    st.markdown(f'<span class="{pill_cls}">{badge_text}</span>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)


def render_signal_card_3(credit_value: float, triggered: bool):
    st.markdown('<div class="signal-card">', unsafe_allow_html=True)
    st.markdown('<div class="signal-eyebrow">Signal 3 — Credit</div>', unsafe_allow_html=True)
    st.markdown('<div class="signal-name">HY − IG spread</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="signal-value">{credit_value:.2f}%</div>', unsafe_allow_html=True)
    st.markdown('<div class="signal-sub">—</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="progress-label-row"><span>Alert threshold</span><span class="threshold-right">{CREDIT_THRESHOLD:.2f}%</span></div>',
        unsafe_allow_html=True,
    )
    render_progress(progress_pct(credit_value, CREDIT_THRESHOLD))
    label = f"{credit_value:.2f}% spread — {'alert' if triggered else 'normal'}"
    pill_cls = "pill pill-danger" if triggered else "pill"
    st.markdown(f'<span class="{pill_cls}">{label}</span>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with dashboard_tab:
    ...
    c1, c2 = st.columns(2)
    with c1:
        render_signal_card_1(snapshot["yield_value"], snapshot["yield_streak"], yield_triggered)
    with c2:
        render_signal_card_2(snapshot["kre_value"], kre_baseline, kre_triggered)

    c3, c4 = st.columns([1, 1])
    with c3:
        render_signal_card_3(snapshot["credit_value"], credit_triggered)
    with c4:
        st.empty()
